...
void solve(){
    ...
    rep(i,0,N-1){
        cin >> A[i];
        uniqueElemntOfA.insert(A[i]);
    }
    ll countMax = 0;
    ll ans;
    for(ll elem : uniqueElemntOfA){
        ll count = 0;
        ll firstTemp,lastTemp;
        rep(i,0,N-1){
            if(elem == A[i]){
                count++;
                if(count == 1){
        ...
        if(max(count,countMax) == count && max(count,countMax) == countMax){
            ans = min(ans, lastTemp - firstTemp + 1);
        }else if(max(count,countMax) != countMax){
            ans = lastTemp - firstTemp + 1;
        }
